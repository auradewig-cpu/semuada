...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const set: { caption?: string; hashtags?: string[]; schedulerAccountId?: string | null } = {};
  ...
  // Reassigning the lane. `undefined` (field absent) leaves it alone; an
  // explicit null returns the video to the shared pool. The category has to be
  // read from the existing row -- see validateLaneAssignment for why a
  // mismatched category would silently strand the video.
  const laneId = laneUpdate(body?.scheduler_account_id);
  if (laneId !== undefined) {
    if (laneId !== null) {
      const [existing] = await db.select().from(videoContents).where(eq(videoContents.id, id));
      if (!existing) return NextResponse.json({ error: "Video tidak ditemukan." }, { status: 404 });
      const invalid = await validateLaneAssignment(laneId, existing.category);
